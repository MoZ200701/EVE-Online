...
## 6. Current Task (Codex) — ▶ P3-0e ACTIVE

**STATUS: P3-0d (`162c4a8`) reviewed DONE (§7) — reviewer re-ran: 131 passed, 1 skipped, ruff/mypy clean (28 files). ACTIVE TASK = P3-0e below — the `backtest` Typer CLI + baseline-comparison report (the FINAL P3-0 slice; closes the harness GATE). Wires `read_price_series` (P3-0d) → the P3-0c engine over the baseline forecasters → `compute_metrics` (P3-0a) side-by-side vs the hold-ISK `0.0` reference. Execute ONLY the P3-0e pack. The `<details>` packs below (P3-0d/P3-0c/P3-0b/P3-0a/M10b/M9c/M9b/M8c) are FINISHED references — do NOT re-execute them. (Reminder: P3-0 needs NO new deps — pure wiring of existing modules + `typer`.) NOTE: the working tree currently has an uncommitted one-line HANDOFF.md edit from Codex (P3-0d §8 `Commit: 162c4a8` hash-fill) — that is correct; it will ride along in the P3-0e commit.**
...
_(Append new entries below — next: P3-0e. P3-0d/P3-0c/P3-0b/P3-0a entries are below.)_
