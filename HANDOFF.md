...
## 6. Current Task (Codex) — ▶ P3-1b ACTIVE

**STATUS: P3-1a (`b5badce`) reviewed DONE (§7) — pure feature leaf landed, leakage gate enforced. ACTIVE TASK = P3-1b below — the store→feature bridge: a `read_history_bars` reader in `store/readers.py` that reads `market_history` → chronological `list[HistoryBar]` (the input `compute_features` consumes). Reader ONLY — no analytics, no CLI, no model. Twin of P3-0d's `read_price_series` but returns the richer `HistoryBar` (all six columns) instead of price-only `PricePoint`. Execute ONLY the P3-1b pack. The `<details>` packs below (P3-1a/P3-0e/P3-0d/P3-0c/P3-0b/P3-0a/M10b/M9c/M9b/M8c) are FINISHED references — do NOT re-execute them. (Reminder: P3-1b needs NO new deps.) Working tree is clean (only untracked scratch `.pytest-tmp*/` + `HANDOFF_ARCHIVE.md` remain, unstaged).**

### P3-1b — `market_history` → `HistoryBar` reader (`store/readers.py`)

The store→feature bridge: read a type's full daily history from `market.duckdb` and return the chronological `HistoryBar` series `compute_features` (P3-1a) consumes. Direct twin of P3-0d's `read_price_series` (same module, same `ensure_market_db` + parameterized-query idiom) — the only differences are the wider column set (all six `market_history` columns, not just `average`) and the `HistoryBar` return type. **Reader ONLY — no analytics, no engine, no CLI, no model.** **No new deps.**

**New-workflow note:** read the **To gather** files for the exact reader idiom; write only the files in scope. The existing reader tests stay green. Anything that changes this design → STOP + §9.

### CONTEXT PACK

**Files in scope (write only these):**
- EDIT `src/evemarket/store/readers.py` — ADD `read_history_bars(...)` (public). ADD `from evemarket.analytics.features import HistoryBar` to the imports (alongside the existing `from evemarket.analytics.backtest import PricePoint` line). **Do NOT alter** `read_station_quotes`, `read_haul_quotes`, `read_price_series`, `_read_daily_volumes`, `_latest_snapshot_path`, `_read_best_quotes`, `_read_type_names`, `_read_type_metadata`, or `_duckdb_string_literal`.
- EDIT `tests/test_readers.py` — ADD history-bar tests; reuse the existing `market_history` fixture helpers (the tmp `market.duckdb` builder + the `executemany` row inserter that `read_price_series`'s tests already use).
...
- `src/evemarket/store/readers.py` — mirror `read_price_series` EXACTLY (lines ~133-158): `data_dir = config.data_dir.expanduser()`; `market_path = data_dir / "market.duckdb"`; `with ensure_market_db(market_path) as connection:`; one parameterized `SELECT ... FROM {MARKET_HISTORY_TABLE} WHERE region_id = ? AND type_id = ? ... ORDER BY date ASC`; map rows in a list comprehension with `row_date.isoformat()` + `float()`/`int()` casts.
- `src/evemarket/analytics/features.py` — confirm `HistoryBar(date: str, average: float, highest: float, lowest: float, order_count: int, volume: int)` field order/types (the return shape; frozen dataclass).
- `tests/test_readers.py` — reuse the `read_price_series` test's `market_history` insertion fixture + tmp-`data_dir` `Config` pattern (out-of-order insert proving the `ORDER BY date ASC`).

**Caller contracts (paste — trust these):**
- `HistoryBar(date: str, average: float, highest: float, lowest: float, order_count: int, volume: int)` — frozen (P3-1a); `compute_features` consumes a **chronological** `Sequence[HistoryBar]` (ascending `date`). The reader MUST return ascending-by-date.
...
- `read_price_series(config, region_id, type_id) -> list[PricePoint]` — the sibling reader; same open/query/map skeleton.
- `Config.data_dir` — the dir holding `market.duckdb`.

**Deliverable — `read_history_bars`:**
```python
...
- Query: `SELECT date, average, highest, lowest, order_count, volume FROM {MARKET_HISTORY_TABLE} WHERE region_id = ? AND type_id = ? AND average IS NOT NULL ORDER BY date ASC` (parameterized `?`). Keep the `average IS NOT NULL` filter (matches `read_price_series` — `average` is the feature "close"; a row with no reference price is unusable for features). `ORDER BY date ASC` guarantees `compute_features`'s chronological precondition.
- Map each row → `HistoryBar(date=row_date.isoformat(), average=float(average), highest=float(highest), lowest=float(lowest), order_count=int(order_count), volume=int(volume))`. (DuckDB `DATE` → Python `date` → `.isoformat()` `YYYY-MM-DD`, same as `read_price_series`.)
- No rows (unknown region/type, or empty db) → `return []` (honest empty; `compute_features([]) == []`).
- No date-window / no min-history params — return the full available series; downstream (`compute_features` warmup, the P3-2 model) handles slicing.

**Conventions to mirror:** clone `read_price_series`'s structure verbatim (open → one parameterized query → list-comprehension map); explicit `float()`/`int()` casts matching `HistoryBar`'s field types; all query **values** parameterized `?`; full type hints; `from __future__ import annotations` already present; terse; **no new deps**; reader-only (no analytics/scan/CLI/model).

**Boundary** — edit only `readers.py` (+ test + §8). NO feature computation (that's `compute_features`, already done — do NOT call it here), NO CLI, NO model, NO change to `features.py`/`backtest.py`/`read_price_series`. Do NOT add date-window/min-history params. Anything that changes the plan → STOP + §9.

**Verification (paste §8, terse per §2) — tests are HERMETIC (tmp `market.duckdb` + `market_history` rows; NO network/live data):**
- Reuse the `read_price_series` test fixtures (tmp `data_dir` `Config`, `market.duckdb` builder, `market_history` `executemany` inserter).
  - **Ascending order + full columns:** insert rows OUT of date order (e.g. `2026-01-03`, `2026-01-01`, `2026-01-02`) → assert the returned `HistoryBar` list is ascending by `date` and each bar's six fields (`average`/`highest`/`lowest`/`order_count`/`volume`) round-trip exactly (with the right `float`/`int` types).
  - **`average IS NULL` excluded:** a row with NULL `average` is dropped from the result (mirror the `read_price_series` NULL test).
  - **Empty/unknown:** unknown type, unknown region, and empty db each → `[]`.
  - **(Integration sanity)** feed the returned list straight into `compute_features(bars, short_window=2, long_window=3)` → runs clean and returns one `FeatureRow` per bar (proves shape compatibility with the P3-1a leaf — the twin of P3-0d's `run_forecaster_backtest` feed-through sanity test).
- `python -m pytest -q` (bundled-Python abs path `C:\Users\M0obo\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe`; AppData temp denied → `--basetemp` at a FRESH dir, e.g. `.pytest-tmp-p31b`). Prior **144 passed, 1 skipped** stays green + new reader tests pass.
- `python -m ruff check .` → clean. `python -m mypy src/` → clean (**still 29 source files** — only `readers.py` edited, no new module).
- Pre-commit `git status --short`: only `src/evemarket/store/readers.py`, `tests/test_readers.py`, `HANDOFF.md` (untracked `HANDOFF_ARCHIVE.md` + `.pytest-tmp*/` unrelated — do NOT stage); no `data/`/`*.duckdb`/parquet. Commit `feat: market_history → HistoryBar reader (P3-1b)`; `git push origin main` (no force).

When done: append §8 entry (terse, **INCLUDE the commit hash + the `read_price_series` idiom you mirrored**) and STOP. After P3-1b → **P3-2** (the forecast model) — Claude will scope it; that step opens the ML-dep sign-off gate (§5/§7 "no new deps"). Do NOT start it or add any ML lib.

<details><summary>Completed — P3-1a: point-in-time feature leaf (reference)</summary>
...
**Recent verdicts:**
- **P3-1a REVIEW: DONE — point-in-time feature leaf landed; the leakage gate is enforced.** `analytics/features.py` + `tests/test_features.py` match the pack. Reviewer re-ran locally (fresh `--basetemp`) → `test_features.py` **8 passed**, full suite **144 passed, 1 skipped** (prior 136 + 8 new), ruff clean, mypy clean (**29** files — `features.py` added). Verified PURE leaf: stdlib-only (`datetime.date`, `statistics.mean/stdev`, `dataclasses`, `collections.abc.Sequence`); NO `evemarket`/Config/store/fees/I-O/model imports; `HistoryBar{date,average,highest,lowest,order_count,volume}` + `FeatureRow{date,simple_return,realized_vol,momentum,price_zscore,volume_ratio,hl_range,day_of_week,day_of_month}` both frozen; `_require_windows` raises `ValueError` up front. **THE leakage gate is correct + explicitly tested:** `simple_returns` is precomputed once but `_simple_return(bars,i)` reads only `bars[i]`/`bars[i-1]`, and every windowed helper (`_realized_vol`/`_momentum`/`_price_zscore`/`_volume_ratio`) slices a **trailing** window ending at `i` — so the load-bearing `compute_features(bars)[t] == compute_features(bars[:t+1])[-1] ∀t` test passes (no future leakage). Hand-verified each value on `[100,110,132,165]` (sw=2,lw=3) at t=2: `simple_return 0.2`, `realized_vol = stdev([0.1,0.2]) ≈ 0.07071`, `momentum 132/100−1 = 0.32`, `price_zscore (132−114)/√268 ≈ 1.0995`, `volume_ratio 300/250 = 1.2`, `hl_range 26/132`; warmup `None`s (t=0 all-`None` except `hl_range`/calendar; t=1 `simple_return 0.1`, vol/momentum/zscore `None`); calendar (`2026-01-05` Mon → `day_of_week 0`, `day_of_month 5`); degenerate guards (flat window → `price_zscore None`; zero-volume → `volume_ratio None`; `average≤0` bar → `simple_return`/`momentum`/`hl_range` `None`); empty → `[]`; `short_window=0`/`long_window=0` → `ValueError`. `realized_vol` needs `start≥1` (excludes the t=0 `None` return) so it gets exactly `short_window` *valid* returns = `short_window+1` bars — matches the spec count. Git `b5badce` (+ hash-fill docs `253ce9d`); scoped to the 3 files, no `data/`. **Deviations (both correct, honestly logged §8):** (a) `_simple_return` also returns `None` when `average_t ≤ 0` (pack only mandated the `average_{t-1}≤0` guard) — defensible honesty: a zero/negative-price bar yields `None`, not a misleading −100% return, and is consistent with the `momentum`/`hl_range` `average>0` guards; (b) `short_window=1` ⇒ `realized_vol` always `None` (sample `stdev` needs ≥2 points) — an honest consequence of sample variance, not a bug. **P3-1 feature leaf to-standard → unblocks P3-1b** (`read_history_bars` reader: `market_history` → `HistoryBar`s).
...
_(Append new entries below — next: P3-1b (`read_history_bars` reader). P3-1a/P3-0e/P3-0d/P3-0c/P3-0b/P3-0a entries are below.)_

### P3-1b - market_history HistoryBar reader - 2026-07-02 - COMPLETE
- Files: `src/evemarket/store/readers.py`, `tests/test_readers.py`, `HANDOFF.md`.
- Gathered/read: `src/evemarket/store/readers.py` `read_price_series` idiom (`config.data_dir.expanduser()` -> `market.duckdb` -> single `with ensure_market_db(market_path)` -> parameterized `?` query -> `ORDER BY date ASC` -> list-comprehension casts); `src/evemarket/analytics/features.py` (`HistoryBar` field order/types, `compute_features` compatibility); `tests/test_readers.py` tmp `Config(data_dir=tmp_path)` + `_write_price_history` fixture style.
- Commands+result:
  - `C:\Users\M0obo\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe -m pytest tests\test_readers.py -q --basetemp .pytest-tmp-p31b-readers` -> `19 passed, 1 warning` (pytest cache permission warning only).
  ...
  - `C:\Users\M0obo\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe -m pytest -q --basetemp .pytest-tmp-p31b-full` -> `148 passed, 1 skipped, 1 warning` (pytest cache WinError 5 only).
