...
## 6. Current Task (Codex) — ▶ M10a ACTIVE

**STATUS: M9 + Phase-2 scanners COMPLETE end-to-end (`c8bae2d`). M10 (Streamlit dashboard) is now decomposed: M10a packaging + skeleton + station-trade panel + AppTest harness → M10b haul panel. EXECUTE the M10a pack directly below. The `<details>` packs further down (M9c/M9b/M8c) are FINISHED references — do NOT execute them.**

### M10a — Streamlit `[ui]` extra + skeleton + station-trade panel

First browser-visual milestone. CREATE the optional Streamlit dashboard package and render the **station-trade scanner** in it, plus establish a hermetic `streamlit.testing` harness. Haul panel is M10b — do NOT add it here. **Pure presentation only:** load config, read quotes, scan, render — ZERO analytics/I-O logic of its own (same discipline as `scan_command`). Reuse `read_station_quotes` + `scan_station_trades`; do NOT reimplement or wrap their math.

**New-workflow note:** read the **To gather** files for exact signatures + the `scan_command` flow to mirror, and consult the **`streamlit.testing.v1.AppTest`** API (less-familiar harness — read streamlit's own docstrings/source for it). Write only the files in scope. Anything that changes this design, or if the AppTest harness proves infeasible → STOP + §9 (do NOT fall back to a browser/Selenium test or invent a data-access layer).

### CONTEXT PACK

**Files in scope (write only these):**
- EDIT `pyproject.toml` — ADD a `[project.optional-dependencies]` **`ui`** extra = `["streamlit"]` (alongside the existing `dev` extra; do NOT add `streamlit` to core `dependencies`, do NOT touch `dev`/`scripts`/build config). Mirror the existing `dev = [...]` block formatting.
- CREATE `src/evemarket/ui/__init__.py` — empty package marker (mirror other `__init__.py` in the tree; a module docstring line is fine, no exports needed).
- CREATE `src/evemarket/ui/app.py` — the Streamlit script (station-trade panel only). `import streamlit as st` at module top is expected/allowed here (this is the one browser surface).
- CREATE `tests/test_ui_app.py` — hermetic `AppTest` tests, guarded by `pytest.importorskip("streamlit")` at top so the suite still passes when the `[ui]` extra is NOT installed.
- EDIT `HANDOFF.md` §8 (log).
- Do NOT touch `cli.py`, `store/readers.py`, `analytics/*`, `config.py`, or anything else.

**To gather (read yourself — do not edit):**
- `src/evemarket/cli.py` — mirror `scan_command`'s flow EXACTLY (lines ~316-397): `load_config(config)`, `region or loaded_config.tracked_regions[0]`, `station if station is not None else loaded_config.home_hub_station_id`, the `read_station_quotes(..., volume_window_days=)` → `scan_station_trades(..., min_roi/min_unit_profit/min_daily_volume/limit)` call, and the two empty-state messages ("No market snapshot found for region {r}. Run ingest-orders first." / "No station-trade opportunities met the filters."). The dashboard is the GUI twin of this command.
- `tests/test_cli_scan.py` — reuse its hermetic fixture approach VERBATIM (tmp `data_dir`; `write_orders_snapshot` + `record_ingest_run(source='esi_orders', status='success', snapshot_path=...)`; `market_history` rows; a tmp `sde.duckdb` with `sde_types(type_id, type_name, volume)`; a `config.toml` in tmp pointing `data_dir` at that dir). **Use region `10000002` + station `60003760`** so the app's auto-resolved defaults (`tracked_regions[0]` / `home_hub_station_id`) hit the fixture data without overriding widgets.
- streamlit's `streamlit.testing.v1.AppTest` — for `AppTest.from_file(path)`, setting widget values via keyed widgets (`at.text_input(key=...).set_value(...)`), `.run()`, and reading `at.dataframe` / `at.info` / `at.error` / `at.exception`. Read the installed package's source if docs are thin.

**Caller contracts (paste — trust these):**
- `load_config(path: str | Path = "config.toml") -> Config` — missing file → all-default `Config`.
- `Config`: `tracked_regions: list[int]` (default `[10000002]`), `home_hub_station_id: int` (default `60003760`), `data_dir: Path` (default `./data`).
- `read_station_quotes(config: Config, region_id: int, station_id: int, *, volume_window_days: int = 30) -> list[MarketQuote]` — sync; `[]` when no snapshot (creates an empty market.duckdb if absent, never raises on missing data); `ValueError` if `volume_window_days < 1`.
- `scan_station_trades(quotes, config, *, min_roi=0.0, min_unit_profit=0.0, min_daily_volume=0.0, limit=None) -> list[StationTradeResult]` — `ValueError` on negative thresholds / `limit<1`.
- `StationTradeResult(type_id:int, type_name:str, buy_price:float, sell_price:float, spread:float, unit_profit:float, roi:float, daily_volume:float)` — `roi` is a fraction.

**Deliverable — `src/evemarket/ui/app.py` (top-to-bottom script, runs on every interaction):**
- `from __future__ import annotations`; `import streamlit as st`; import `load_config`, `read_station_quotes`, `scan_station_trades`, `StationTradeResult`.
- `st.title("EVE Market Tool")` (or `st.set_page_config` + title — keep minimal).
- **Sidebar inputs** (all keyed so AppTest can set them):
  - `config_path = st.sidebar.text_input("Config file", value="config.toml", key="config_path")` → `loaded_config = load_config(config_path)` immediately after (so the region/station defaults below come from it).
  - `region = st.sidebar.number_input("Region ID", value=loaded_config.tracked_regions[0], step=1, key="region")` (cast to `int`).
  - `station = st.sidebar.number_input("Station ID", value=loaded_config.home_hub_station_id, step=1, key="station")` (cast to `int`).
  - `min_roi` (default `0.0`), `min_unit_profit` (`0.0`), `min_daily_volume` (`0.0`) as `st.sidebar.number_input(..., value=0.0, key=...)`.
  - `limit = st.sidebar.number_input("Limit", value=20, min_value=1, step=1, key="limit")` (cast `int`).
  - `volume_window_days = st.sidebar.number_input("Volume window (days)", value=30, min_value=1, step=1, key="volume_window_days")` (cast `int`).
- **Station Trading panel:** `st.header("Station Trading")`.
  - `quotes = read_station_quotes(loaded_config, int(region), int(station), volume_window_days=int(volume_window_days))`.
  - `results = scan_station_trades(quotes, loaded_config, min_roi=min_roi, min_unit_profit=min_unit_profit, min_daily_volume=min_daily_volume, limit=int(limit))`.
  - `st.caption(f"Region: {int(region)}  Station: {int(station)}  Quotes: {len(quotes)}")`.
  - `if not quotes:` → `st.info(f"No market snapshot found for region {int(region)}. Run ingest-orders first.")`
  - `elif not results:` → `st.info("No station-trade opportunities met the filters.")`
  - `else:` → render a table. Build `rows = [dataclasses.asdict(r) for r in results]` (or a list of dicts with the 8 `StationTradeResult` fields in order) and `st.dataframe(rows, use_container_width=True)`. **No pandas** (not a dep) — `st.dataframe` accepts a `list[dict]`; a `polars.DataFrame` is also acceptable (polars IS a dep). Keep raw numeric columns (no `,.2f` formatting — that's M10b polish).
- NO haul code, NO `st.cache_data`, NO analytics. Full type hints where the script form allows; terse.

**Conventions to mirror:** pure presentation (the only logic is the same read→scan wiring `scan_command` does); reuse readers/scanners as-is; `streamlit` ONLY via the `[ui]` extra (never core); no other new deps; `from __future__ import annotations`.

**Boundary** — gather only the named files; write only the 4 in scope; do NOT modify the reader/scanner/CLI/config or add a data-access layer. Station-trade panel ONLY (haul is M10b). Design change or infeasible AppTest harness → STOP + §9.

**Verification (paste §8, terse per §2) — tests HERMETIC (tmp fixtures + `AppTest`, NO network/live data, NO real browser):**
- `tests/test_ui_app.py` opens with `import pytest` then `pytest.importorskip("streamlit")` (module-level skip when `[ui]` not installed). `from streamlit.testing.v1 import AppTest`. Resolve the app path as `Path(evemarket.ui.app.__file__)` or repo-relative `src/evemarket/ui/app.py`.
- **Empty state (hermetic):** build a tmp `config.toml` whose `data_dir` is an EMPTY tmp dir (no market.duckdb). `at = AppTest.from_file(<app path>)`; `at.text_input(key="config_path").set_value(str(cfg))`; `at.run()`. Assert `not at.exception`; assert some `st.info` element's value contains `"No market snapshot"`. (Do NOT rely on the repo's real `config.toml`/`./data` — always point `config_path` at a tmp config so the test is hermetic.)
- **Happy path:** build the populated fixture (snapshot region `10000002` / station `60003760`: type 34 buy@100/sell@120 + type 35 sell-only; `market_history`; tmp `sde.duckdb`). Set `config_path` to that tmp config; `at.run()`. Assert `not at.exception`; assert a `st.dataframe` rendered; assert its data contains `34`/`Tritanium` and does NOT show type 35 (no bid → scanner skips). (region/station auto-default to the fixture values — no widget override needed.)
- **Filter excludes all:** populated fixture but set `min_roi` widget to `999` (`at.number_input(key="min_roi").set_value(999.0)`), `at.run()` → assert an `st.info` contains `"No station-trade opportunities"`.
- `python -m pytest -q` (bundled-Python abs path `C:\Users\M0obo\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe`; AppData temp denied → `--basetemp .pytest-tmp` at a fresh dir). **If `streamlit` is not installed in the runtime, install the `[ui]` extra** (`...python.exe -m pip install -e ".[ui]"`) so the new tests actually RUN, not skip — report which happened in §8. Prior **90 passed, 1 skipped** stays green; new tests pass (or are reported skipped if streamlit truly cannot install — but then note it as a §9 blocker, since an unrun harness isn't verified).
- `python -m ruff check .` → clean. `python -m mypy src/` → clean (now **25** source files: `ui/__init__.py` + `ui/app.py` added; if mypy flags missing `streamlit` stubs, add `streamlit.*` to the existing `[[tool.mypy.overrides]] ignore_missing_imports` list in `pyproject.toml` — that file is in scope).
- Pre-commit `git status --short`: only `pyproject.toml`, `src/evemarket/ui/__init__.py`, `src/evemarket/ui/app.py`, `tests/test_ui_app.py`, `HANDOFF.md` (untracked `HANDOFF_ARCHIVE.md` is an unrelated planner doc — do NOT stage it); no `data/`/`*.duckdb`/parquet/`.pytest-tmp`. Commit `feat: Streamlit dashboard skeleton + station-trade panel (M10a)`; `git push origin main` (no force). Include `HANDOFF.md`.

When done: append §8 entry (terse, **INCLUDE the commit hash, what you gathered from `scan_command`/`AppTest`, and whether streamlit installed + tests RAN vs skipped**) and STOP. After M10a → **M10b** (add the haul panel to `ui/app.py` + extend tests; mirrors `haul_command`) → M10 complete (both scanners in one dashboard).
...
_(Append new entries below — next: M10a.)_
